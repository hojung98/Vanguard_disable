...
def disable_startup_program(program_name):
    try:
        ...
        try:
            ...
            print(f"{program_name}이 시작 프로그램 목록에 존재하지 않아요 ㅠㅠㅠ")

        print(f"{program_name}을 시작프로그램 목록에서 제외했어요. 컴퓨터를 껐다 켜시면 적용될꺼에요! 게임 실행 시에는 자동으로 활성화되니 제재당하실 걱정은 안 하셔도 되어요!!")
    except FileNotFoundError:
        print("뱅가드 레지스트리를 찾을 수 없어요 ㅠㅠㅠㅠㅠ")
    except Exception as e:
        print(f"오류가 발생했어요! 혹시 관리자 권한으로 실행하셨나요? 관리자 권한으로 실행해야 되어요 ㅠㅠ 아니면 보이는 오류 코드와 함께 저에게 문의해 주세요!: {e}")

    input("\n엔터를 누르면 종료됩니당")  # 결과 확인 후 종료
